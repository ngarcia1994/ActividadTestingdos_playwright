...
test('Probar componente text box y formulario', async ({ page }) => {
  ...
  await page.getByRole('heading', { name: 'Contenido' }).click();
  await page.locator("//span[contains(text(), 'Text Box')]").click();
  await page.getByRole('textbox', { name: 'Full Name' }).fill("Nelson Alejandro Garcia");  // campo 1
  await page.getByRole('textbox', { name: 'name@example.com' }).fill("alejandro.garcia94@prueba.com");
  ...
  await expect(page.getByText("Email:alejandro.garcia94@prueba.com")).toBeVisible({ timeout: 8_000 });
...
test('Probar componente text box y formulario GRABANDO', async ({ page }) => {
  ...
  await page.getByPlaceholder('Full Name').fill('Juan Carlos Henao'); // campo 1
  await page.getByPlaceholder('juanheano04@gmail.com').fill('juanheano04@gmail.com');
  await page.getByPlaceholder('Current Address').fill('prueba current');
  await page.locator('#permanentAddress').fill('prueba address');
  await page.getByRole('button', { name: 'Submit' }).click();
  await expect(page.locator('#email')).toContainText('Email:juanheano04@gmail.com');
});
...
test('Probar componente Web table', async ({ page }) => {
  ...
  await page.getByPlaceholder('First Name').fill('Nelson Alejandro');
  await page.getByPlaceholder('Last Name').fill('Garcia Morales');
  await page.getByPlaceholder('name@example.com').fill('alejandro.garcia94@prueba.com');
  ...
  await page.getByRole('button', { name: 'Submit' }).click();
  await expect(page.getByRole('grid')).toContainText('alejandro.garcia94@prueba.com');
});
...
test('Probar componente Web table eliminar registro', async ({ page }) => {
  ...
  await page.locator('(//div[contains(text(),"alejandro.garcia94@prueba.com")]//following::span)[2]').click();
